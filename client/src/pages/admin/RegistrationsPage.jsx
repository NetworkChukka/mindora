...
export const RegistrationsPage = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    setEditLoading(true);
    try {
      const res = await api.put(`/registrations/${editItem._id}`, editForm);
      if (res.data.success) {
        success(`Registration ${editItem.registrationNumber} updated successfully`);
        setEditItem(null);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await api.delete(`/registrations/${deleteItem._id}`);
      if (res.data.success) {
        success(`Registration ${deleteItem.registrationNumber} deleted`);
        setDeleteItem(null);
        fetchRegistrations(pagination.page);
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-3xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Total recorded: <strong>{pagination.total}</strong> students
          </p>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div className="md:col-span-2 relative">
          ...
          <input
            type="text"
            placeholder="Search by student name, MIN ID, school, phone..."
            value={searchTerm}
              ...
              {loading ? (
                <tr>
                  <td colSpan="9" className="py-12">
                    <LoadingSpinner label="Fetching registrations..." />
                  </td>
                </tr>
      ...
      {editItem && (
        <Modal
          ...
          title={`Edit Registration: ${editItem.registrationNumber}`}
          maxWidth="max-w-md"
        >
      ...
      <ConfirmationModal
        ...
        title="Delete Registration"
        message={`Are you sure you want to delete registration ${deleteItem?.registrationNumber} for ${deleteItem?.studentName}? It will be removed from active exhibition statistics.`}
